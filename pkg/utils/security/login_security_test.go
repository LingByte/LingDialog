...
func setupTestLoginSecurityManager(t *testing.T) (*LoginSecurityManager, cache.Cache, func()) {
	...
	lsm := NewLoginSecurityManager(logger, cacheInstance)

	cleanup := func() {
		cacheInstance.Close()
	}

	return lsm, cacheInstance, cleanup
}
...
func TestLoginSecurityManager_CheckAccountLock_NoLock(t *testing.T) {
	lsm, _, cleanup := setupTestLoginSecurityManager(t)
	defer cleanup()
...
func TestLoginSecurityManager_RecordFailedLogin(t *testing.T) {
	lsm, cacheInstance, cleanup := setupTestLoginSecurityManager(t)
	defer cleanup()
...
func TestLoginSecurityManager_ClearFailedLoginCount(t *testing.T) {
	...
	_, ok := cacheInstance.Get(context.Background(), key)
	if ok {
...
func TestLoginSecurityManager_CheckIPRateLimit_NoCache(t *testing.T) {
	logger := zaptest.NewLogger(t)
	lsm := NewLoginSecurityManager(logger, nil) // 传入 nil cache
...
// TestInitGlobalLoginSecurityManager 已移除，因为全局变量已被废弃
