...
const AuthModal = ({ isOpen, onClose, initialMode = 'login' }: AuthModalProps) => {
  ...
  useEffect(() => {
    if (isOpen) {
      systemApi.getSystemInit().then((res: any) => {
  ...
  const handleTwoFactorSubmit = async () => {
    ...
    try {
      // 加密密码
      const encryptedPassword = encryptPasswordToString(formData.password)
      ...
      if (response.code === 200) {
        // 获取token：优先从data中获取
        const token = response.data.token || (response.data.user as any)?.token || (response.data.user as any)?.authToken || (response.data.user as any)?.AuthToken
        if (!token) {
        ...
        if (loginSuccess) {
          ...
          const displayName = response.data.user?.displayName || (response.data.user as any)?.DisplayName || (response.data as any)?.displayName || formData.email
  ...
  // 执行登录
  const performLogin = async (_captchaId: string, _captchaCode: string) => {
    setIsLoading(true)
    try {
        if (loginType === 'email') {
          // 邮箱验证码登录
          ...
          if (response.code === 200) {
            ...
            // 从data中获取详细错误信息
            const errorMessage = (response.data as any)?.message || response.msg || '登录失败'
            throw new Error(errorMessage)
          }
        } else {
        // 密码登录 - 需要验证码
          ...
          if (response.code === 200) {
            ...
            // 获取token：优先从data中获取
            const token = response.data.token || (response.data.user as any)?.authToken || (response.data.user as any)?.AuthToken
            if (!token) {
              throw new Error('登录成功但未获取到认证令牌，请重试')
            }
